...
RSpec.describe Packer::SysprepScriptGenerator do
  ...
  describe "#content" do
    ...
    context "installing openssh" do
      let(:script) { generator.content(iaas: :gcp) }

      it "includes openssh stanza" do
        expect(script).to include("Write-Log \"Install OpenSSH.Server\"")
        expect(script).to include("Add-WindowsCapability -Online -Name (Get-WindowsCapability -Online -Name \"OpenSSH.Server*\" | ForEach-Object Name)")
      end
    end
