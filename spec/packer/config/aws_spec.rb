      ...
      it 'returns the expected provisioners' do
        ...
        expect(provisioners).to eq(
          [
            ...
            {"type"=>"windows-restart", "restart_command"=>"powershell.exe -Command bcdedit -set testsigning on; C:\\provision\\private-patch1.exe /q ; start-sleep 300; restart-computer", "restart_timeout"=>"1h"},
            {'type'=>'powershell', 'inline'=>['$ErrorActionPreference = "Stop";',
                                              'trap { $host.SetShouldExit(1) }',
                                              'Set-ProxySettings   ']},
            {"type"=>"powershell", "inline"=> ["$ErrorActionPreference = \"Stop\";", "trap { $host.SetShouldExit(1) }", "New-Provisioner"]},
