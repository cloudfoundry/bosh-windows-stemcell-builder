...
describe Stemcell::Publisher::Azure do
  before(:each) do
    ...
    azure_published_storage_account = 'some-azure_published_storage_account'
    ...
    @publisher = Stemcell::Publisher::Azure.new(
      version: version, sku: sku, azure_storage_account: azure_storage_account, azure_published_storage_account: azure_published_storage_account,
  ...
  describe '#print_publishing_instructions' do
    ...
    it 'prints instructions for publishing azure image' do
      ...
      expected_sas_command = "az storage container generate-sas --name #{@publisher.container_name} --permissions rl --account-name #{@publisher.azure_published_storage_account} --account-key #{@publisher.azure_published_storage_access_key} --start #{validFrom} --expiry #{validTo}"
      expected_url_command = "az storage blob url --container-name #{@publisher.container_name} --name #{@publisher.container_path} --account-name #{@publisher.azure_published_storage_account} --account-key #{@publisher.azure_published_storage_access_key}"
...
END
  ...
  describe '#copy_vhd_to_published_storage_account' do
    it 'copies vhd from unpublished storage account to published storage account' do
      expected_login_command = "az login --username #{@publisher.azure_client_id} --password #{@publisher.azure_client_secret} --service-principal --tenant #{@publisher.azure_tenant_id}"
      expected_copy_command = "az storage blob copy start --source-account-key \"#{@publisher.azure_storage_access_key}\" --source-account-name \"#{@publisher.azure_storage_account}\" --source-container \"system\" --source-blob \"#{@publisher.container_path}\" --account-name \"#{@publisher.azure_published_storage_account}\" --destination-container \"system\" --destination-blob \"#{@publisher.container_path}\""
