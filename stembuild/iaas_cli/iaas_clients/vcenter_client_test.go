...
func vcenterAuthUrl(vcenterUsername, vcenterPassword, vcenterUrl string) string {
	return fmt.Sprintf("%s:%s@%s", vcenterUsername, vcenterPassword, vcenterUrl)
}

var _ = Describe("VcenterClient", func() {
	var runner *iaas_clifakes.FakeCliRunner

	BeforeEach(func() {
		runner = &iaas_clifakes.FakeCliRunner{}
	})

	Describe("NewVcenterClient", func() {
		var (
		...
		BeforeEach(func() {
			vcenterUsername = `special\chars!user#`
			vcenterPassword = `special\chars!user#`
			vcenterUrl = "fake-url"

			encodedUsername = url.QueryEscape(vcenterUsername)
			encodedPassword = url.QueryEscape(vcenterPassword)
		})

		It("url encodes credentials with special characters", func() {
			vcenterClient := iaas_clients.NewVcenterClient(vcenterUsername, vcenterPassword, vcenterUrl, "", runner)

			expectedArgs := []string{"about", "-u", vcenterAuthUrl(encodedUsername, encodedPassword, vcenterUrl)}
		...
		Context("A ca cert file is specified", func() {
			It("Passes the ca cert to govc", func() {
				vcenterClient := iaas_clients.NewVcenterClient(vcenterUsername, vcenterPassword, vcenterUrl, "fake-cert-file.txt", runner)

				expectedArgs := []string{"about", "-u", vcenterAuthUrl(encodedUsername, encodedPassword, vcenterUrl), "-tls-ca-certs=fake-cert-file.txt"}
	...
	Describe("VcenterClient", func() {
		var (
			vcenterClient          *iaas_clients.VcenterClient
			vcenterUsername        string
			vcenterPassword        string
			vcenterUrl             string
			vcenterCertificateFile string
		)

		BeforeEach(func() {
			vcenterUsername = "fake-vcenter-username"
			vcenterPassword = "fake-vcenter-password"
			vcenterUrl = "example.com/fake-vcenter-url"
			vcenterCertificateFile = ""
		...
		Describe("ValidateCredentials", func() {
			It("When the login credentials are correct, login is successful", func() {
				expectedArgs := []string{"about", "-u", vcenterAuthUrl(vcenterUsername, vcenterPassword, vcenterUrl)}
			...
			It("When the login credentials are incorrect, login is a failure", func() {
				...
				Expect(runner.RunCallCount()).To(Equal(1))
				Expect(argsForRun).To(Equal(expectedArgs))
				Expect(err).To(MatchError(fmt.Sprintf("vcenter_client - invalid credentials for: %s:REDACTED@%s", vcenterUsername, vcenterUrl)))
			})
		})

		Describe("ValidateUrl", func() {
			It("When the url is valid, there is no error", func() {
				expectedArgs := []string{"about", "-u", vcenterUrl}
			...
			It("When the url is invalid, there is an error", func() {
				...
				Expect(runner.RunCallCount()).To(Equal(1))
				Expect(argsForRun).To(Equal(expectedArgs))
				Expect(err).To(MatchError(fmt.Sprintf("vcenter_client - unable to validate url: %s", vcenterUrl)))
			})

			It("a validateUrl failure mentions the ca cert if one was specified", func() {
				vcenterClient = iaas_clients.NewVcenterClient(vcenterUsername, vcenterPassword, vcenterUrl, "fake-cert.txt", runner)
				expectedArgs := []string{"about", "-u", vcenterUrl, "-tls-ca-certs=fake-cert.txt"}
				...
				Expect(runner.RunCallCount()).To(Equal(1))
				Expect(argsForRun).To(Equal(expectedArgs))
				Expect(err).To(MatchError(fmt.Sprintf("vcenter_client - invalid ca certs or url: %s", vcenterUrl)))
			})
		...
		Describe("FindVM", func() {
			It("If the VM path is valid, and the VM is found", func() {
				expectedArgs := []string{"find", "-u", vcenterAuthUrl(vcenterUsername, vcenterPassword, vcenterUrl), "-maxdepth=0", "validVMPath"}
				runner.RunReturns(0)
			...
			It("If the VM path is invalid", func() {
				expectedArgs := []string{"find", "-u", vcenterAuthUrl(vcenterUsername, vcenterPassword, vcenterUrl), "-maxdepth=0", "invalidVMPath"}
				runner.RunReturns(1)
		...
		Describe("RemoveDevice", func() {
			It("Removes a device from the given VM", func() {
				...
				Expect(err).To(Not(HaveOccurred()))
				expectedArgs := []string{"device.remove", "-u", vcenterAuthUrl(vcenterUsername, vcenterPassword, vcenterUrl), "-vm", "validVMPath", "device"}
		...
		Describe("EjectCDRom", func() {
			It("Ejects a cd rom from the given VM", func() {
				...
				Expect(err).To(Not(HaveOccurred()))
				expectedArgs := []string{"device.cdrom.eject", "-u", vcenterAuthUrl(vcenterUsername, vcenterPassword, vcenterUrl), "-vm", "validVMPath", "-device", "deviceName"}
				Expect(runner.RunArgsForCall(0)).To(Equal(expectedArgs))
			})
...
`
			...
			It("returns a list of devices for the given VM", func() {
				...
				Expect(runner.RunWithOutputArgsForCall(0)).To(Equal([]string{"device.ls", "-u", vcenterAuthUrl(vcenterUsername, vcenterPassword, vcenterUrl), "-vm", "/path/to/vm"}))
			})
		...
		Describe("ExportVM", func() {
			...
			It("exports the VM to local machine from vcenter using vm inventory path", func() {
				expectedArgs := []string{"export.ovf", "-u", vcenterAuthUrl(vcenterUsername, vcenterPassword, vcenterUrl), "-sha", "1", "-vm", "validVMPath", destinationDir}
				runner.RunReturns(0)
			...
			It("Returns an error message if ExportVM fails to export the VM", func() {
				...
				expectedArgs := []string{"export.ovf", "-u", vcenterAuthUrl(vcenterUsername, vcenterPassword, vcenterUrl), "-sha", "1", "-vm", vmInventoryPath, destinationDir}
				runner.RunReturns(1)
				err := vcenterClient.ExportVM("validVMPath", destinationDir)
		...
		Describe("UploadArtifact", func() {
			It("Uploads artifact to the given vm", func() {
				...
				Expect(err).To(Not(HaveOccurred()))
				expectedArgs := []string{"guest.upload", "-u", vcenterAuthUrl(vcenterUsername, vcenterPassword, vcenterUrl), "-f", "-l", "user:pass", "-vm", "validVMPath", "artifact", "C:\\provision\\artifact"}
				Expect(runner.RunArgsForCall(0)).To(Equal(expectedArgs))
			})
		...
		Describe("MakeDirectory", func() {
			It("Creates the directory on the vm", func() {
				...
				Expect(err).To(Not(HaveOccurred()))
				expectedArgs := []string{"guest.mkdir", "-u", vcenterAuthUrl(vcenterUsername, vcenterPassword, vcenterUrl), "-l", "user:pass", "-vm", "validVMPath", "-p", "C:\\provision"}
				Expect(runner.RunArgsForCall(0)).To(Equal(expectedArgs))
			})
		...
		Describe("Start", func() {
			It("runs the command on the vm", func() {
				...
				expectedArgs := []string{"guest.start", "-u", vcenterAuthUrl(vcenterUsername, vcenterPassword, vcenterUrl), "-l", "user:pass", "-vm", "validVMPath", "command", "arg1", "arg2", "arg3"}
		...
		Describe("WaitForExit", func() {
			...
			It("returns the process' exit code upon success", func() {
				...
				expectedArgs := []string{"guest.ps", "-u", vcenterAuthUrl(vcenterUsername, vcenterPassword, vcenterUrl), "-l", "user:pass", "-vm", "validVMPath", "-p", "1296", "-X", "-json"}
				Expect(runner.RunWithOutputCallCount()).To(Equal(1))
				Expect(runner.RunWithOutputArgsForCall(0)).To(Equal(expectedArgs))
			})
		...
		Describe("IsPoweredOff", func() {
			It("Uses vm.info correctly to get power state", func() {
				expectedArgs := []string{"vm.info", "-u", vcenterAuthUrl(vcenterUsername, vcenterPassword, vcenterUrl), "validVMPath"}
